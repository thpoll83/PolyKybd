...
cutouts2 = "poly_kybd_split72_plate_right-Edge_Cuts.svg";
cutouts = "cut-outs.svg";
...
module pcb_shape(extra_radius = 0) {
    radius = case_wall_thickness + pcb_clearance+ extra_radius;
    linear_extrude(height=0.5, center=true) offset(r = radius+2, $fn = 128) offset(r = -2, $fn = 128) import(file = pcb_outline, dpi = 300, center=true);
}

module pcb_shape_camfer_top(extra_offet = 0) {
    for ( i = [0 : 90] ){
        translate([0,extra_offet*i,cos(i)]) pcb_shape(extra_radius = sin(i));
    }
}
module pcb_shape_camfer_bottom() {
    for ( i = [0 : 90/5] ){
        translate([0,0,1-cos(i*5)]) pcb_shape(extra_radius = sin(i*5));
    }
}
...
module right_side_modular(mirror_text, fdm_print, with_shrink_protection, grid_infill = true)
{
   ...
   difference() {
        //base shape
        hull() {
            translate([0,0,case_height]) pcb_shape_camfer_top(extra_offet=0);
    
            scale(1.08) translate([0,0,-5.5]) pcb_shape();
        }
        //wedge cut-off
        translate([0,0,-10]) rotate([-5,0,0]) cube([300,300,20],center=true);
        
        //inner hollow space
        translate([0,0,-10-1.3]) linear_extrude(height=case_height+8.3-0.85) hull() offset(r = pcb_clearance+0.5, $fn = 128) import(file = pcb_outline, dpi = 300, center=true);
        
        //inner border
        translate([0,0,-10]) linear_extrude(height=case_height+8.3-0.85) offset(r = pcb_clearance+0.5-4, $fn = 128) import(file = pcb_outline, dpi = 300, center=true);
   
        //clearance around switches
        translate([-4.215,0.779,case_height+6.3-1.5-10]) linear_extrude(height=5) rotate([0,180,0]) offset(r=1.2) import(file = cutouts, dpi = 300, center=true);
    
        //switch cut-outs
        color([1,0,0]) translate([-4.215,0.779,0]) linear_extrude(height=100) rotate([0,180,0]) import(file = cutouts, dpi = 300, center=true);
        
        //extra space around display
        translate([-75,21.5,15.4]) cube([30,69,5], center=true);
    

translate([-92,-72,1]) {
        // cut out LEDs
        translate([ 0, 0, pcb_edge_height - 0.1-0.5 ]) linear_extrude(height = 2.20, scale = 1) offset(r = 0.9)
            import(file = led_holes, dpi = 300);

        // cut out switch
        translate([ 0, 0, pcb_edge_height - 0.4-1 ]) linear_extrude(height = 3, scale = 1) offset(r = 2.5)
            import(file = switch_holes, dpi = 300);    

        // cut out USB
        translate([ 0, 0, pcb_edge_height - 1.6-2 ]) linear_extrude(height = 8, scale = 1) offset(r = +1.1, $fn = 50)
...
/*
 ...
 //linear_extrude(height=100) rotate([0,180,0]) offset(r=0) import(file = cutouts2, dpi = 300, center=true);
