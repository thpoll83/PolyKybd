...
"""Verify the one-piece LED diffuser frame against the things that have bitten it.
    ...
    python3 parts/diffuser/check_frame.py            # every check, both halves
    python3 parts/diffuser/check_frame.py --quick    # skip the OpenSCAD boolean checks
...
def intersect_volume(placement, workdir):
    ...
    src = ('use <case_polykybd_split72_lr.scad>\n'
           'use <../diffuser/diffuser_frame_left.scad>\n'
...
def revision_zone(side):
    ...
    p = os.path.join(SRC, f'diffuser_frame_{side}.scad')
    with open(p) as fh:
        src = fh.read()
...
def check_meshes():
    ...
    for side in ('left', 'right'):
        p = os.path.join(EXPORT, f'diffuser_frame_{side}.stl')
...
def check_mirror():
    ...
    ps = [os.path.join(EXPORT, f'diffuser_frame_{s}.stl') for s in ('left', 'right')]
...
def check_plate_trap():
    ...
    with open(os.path.join(SRC, 'diffuser.scad')) as fh:
        src = fh.read()
