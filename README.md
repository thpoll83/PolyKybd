...
0. **(Optional but highly recommended) Solder the LTR-559 light + proximity sensor - do this FIRST!**
   ...
   ⚠️ Each half has a **single** expansion slot shared by the sensor, the rotary encoder and a pointing device (trackball/trackpad) - only **one** of the three fits per half. So fitting the sensor means no encoder or trackpad on that half (you can still fit one of those on the other half). Decide before you start soldering.
